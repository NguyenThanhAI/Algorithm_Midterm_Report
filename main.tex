...
\begin{document}
    ...
    \begin{algorithm}[h!]
        ...
        \Repeat{đạt điều kiện dừng}{
            \ForEach{$X \in P$}{
                $C(X) \gets Cost(X)$\;
                $P^{\prime} \gets$ các phần tử phù hợp nhất từ $P$\;
                $P^{\prime\prime} \gets \emptyset$\;
                \Repeat{đủ số lượng phần tử thế hệ sau được sinh ra}{
                    Chọn $X_1$ và $X_2$ một cách ngẫu nhiên từ $P^{\prime}$\;
                    Lai tạo $X_1$ và $X_2$ tạo ra $X_{child}$\;
                    $P^{\prime\prime} \gets P^{\prime\prime} \cup X_{child}$
                }
                \ForEach{$X \in P^{\prime\prime}$}{
                    Áp dụng phép đột biến vào $X$\;
                }
            }
            $P \gets P^{\prime \prime}$\;
        }
        \Return{$\underset{X \in P}{\mathrm{argmax}} \thickspace C(X)$}
